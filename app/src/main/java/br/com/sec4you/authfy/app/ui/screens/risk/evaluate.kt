...
fun Evaluate(
  ...
  Scaffold(
    ...
    bottomBar = { HomeFooter(navController, currentRoute = "risk") }
  ) { innerPadding ->
    Column(
      modifier = Modifier
        .fillMaxSize()
      ...
      Button(
        ...
        onClick = {
          if (accessToken == null) {
            Toast.makeText(context, "No valid token found. Please login again.", Toast.LENGTH_LONG)
              .show()
            return@Button
          }
  ...
  when(val result = NetworkUtils.doPost(
    ...
    is NetworkUtils.NetworkResult.Success -> {
      if (postEvaluate) {
        ...
        Log.d("AUTHCUBE:EVALUATE", "Success requesting evaluate. Going for post-evaluate...")
        ...
        when (val postEvaluateResult = NetworkUtils.doPost(
          ...
          is NetworkUtils.NetworkResult.Success -> {
            Log.d("AUTHCUBE:POST_EVAL", "post-evaluate response: ${postEvaluateResult.data}")
          ...
          is NetworkUtils.NetworkResult.Exception -> {
            Log.e("AUTHCUBE:POST_EVAl", "Failed requesting post-evaluate.", postEvaluateResult.e)
          }
        }
